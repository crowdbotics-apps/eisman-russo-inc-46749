...
class EventCreateSerializer(serializers.ModelSerializer):
                ...
                try:
                    ...
                    if "unique_event_start_date_end_date" in str(e):
                        raise serializers.ValidationError(
                            {
                                "fema_dates": [
                                    "A Fema date with the same start date and end date already exists for this "
                                    "event."
                                ]
                            }
                        )
    ...
    def update(self, instance, validated_data):
        ...
        if fema_dates_data:
            ...
            for fema_date_data in fema_dates_data:
                ...
                if "id" in fema_date_data:
                    fema_date = FemaDates.objects.get(
                        id=fema_date_data["id"], event=instance
                    )
                    fema_date.start_date = fema_date_data.get(
                        "start_date", fema_date.start_date
                    )
                    fema_date.end_date = fema_date_data.get(
                        "end_date", fema_date.end_date
                    )
                    ...
                    try:
                        ...
                        if "unique_event_start_date_end_date" in str(e):
                            raise serializers.ValidationError(
                                {
                                    "fema_dates": [
                                        "A Fema date with the same start date and end date already exists for "
                                        "this event."
                                    ]
                                }
                            )
                            ...
                            raise serializers.ValidationError(
                                {
                                    "fema_dates": [
                                        "An error occurred while updating the Fema date."
                                    ]
                                }
                            )
                else:
                    try:
                        fema_date_create_instances.append(
                            FemaDates(event=instance, **fema_date_data)
                        )
                    except IntegrityError as e:
                        if "unique_event_start_date_end_date" in str(e):
                            raise serializers.ValidationError(
                                {
                                    "fema_dates": [
                                        "A Fema date with the same start date and date already exists for this "
                                        "event."
                                    ]
                                }
                            )
        ...
        elif "fema_dates" in self.initial_data and (
            fema_dates_data is None or fema_dates_data == []
        ):
